...
Every phase file in `v1_0/` is written against placeholder tokens instead of one author's folders. Replace them once, and the whole pipeline points at your book.
...
| `<RAG_COLLECTION>` | Vector store collection name for the whole book | `quiet_engine_book` |
...
| **Local models** | `Qwen2.5 14B Instruct` (drafting), `nomic-embed-text` (embedding) | Defaults, not requirements. Keep one embedding model for the whole book, whichever you pick — changing it means re-embedding every chapter. |
| **Toolchain** | Claude and Claude Design (generation), InDesign (print), XTTS (narration), Ollama + ChromaDB (local RAG), n8n + OpenClaw (orchestration) | Swap freely. The gates, deliverables, and source-of-truth rules are the pipeline; the tools are an implementation. |
