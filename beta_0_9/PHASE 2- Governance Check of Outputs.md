# PHASE 2: GOVERNANCE CHECK + AUTO-REMEDIATION (v2)
...
- File naming violations (`<ChapterName>_<ArtifactType>.<ext>`) and missing required folders
...
- Required folders exist, including `/Marketing/Intake/`, `/Marketing/Seeds/`, and `/Marketing/Trigger/`
...
**A chapter may not be certified Production Ready or higher while any Critical issue is open or any `Needs Author Review` item is unresolved.** In those cases, certify at the highest level the evidence supports and list what blocks the next level.
...
```text
/Documents/Books/RCF_FIRST_EDITION/RCF/Chapters/_BookGovernance/Ch<first>-<last>_CrossChapterConsistencyReport.md
```
